            ...
            for (int i = 0; i < activities.size(); i++) {
                for (int j = 0; j < sensors.size(); j++) {
                    ...
                    try {
                        ...
                        outputStream.write("Time(ms),X(mG),Y(mG),Z(mG)\n".getBytes());
                        // Write all the readings which correlate to our current activity
                        for (int k = 0; k < readings.size(); k++) {
                            //Had to comment out this if statement or no data would be saved on phone
                            //if (readings.get(k).timestamp >= activities.get(i).startTime && readings.get(k).timestamp < activities.get(i).endTime) {
                                ...
                                outputStream.write(String.format(Locale.US, "%14d,%+5d,%+5d,%+5d\n", readings.get(k).timestamp, readings.get(k).x, readings.get(k).y, readings.get(k).z).getBytes());
